...
class LSTMModel(nn.Module):
    def __init__(self, input_size, hidden_sizes, output_size, num_layers, dropout):
        ...
        self.lstms.append(nn.LSTM(input_size,
                                  hidden_sizes[0],
                                  num_layers=num_layers,
                                  dropout=dropout,
                                  batch_first=True))
        for i in range(1, len(hidden_sizes)):
            self.lstms.append(nn.LSTM(hidden_sizes[i-1],
                                      hidden_sizes[i],
                                      num_layers=num_layers,
                                      dropout=dropout,
                                      batch_first=True))
...
class Model:
    ...
    def train_new(self, channel):
        ...
        if self.config.model_architecture == "LSTM":
            ...
            self.reserviors = []
            self.reserviors.append(esn.Reservoir(
                                    self.input_size,
            ...
            if channel.train_with_val:
                self.W_readout, self.l2_values, best_mse = ridge_regression.fit_and_validate_readout(
    ...
    def batch_predict(self, path, channel):
        ...
        for X_test_batch, y_hat_batch in channel.test_loader:
            X_test_batch, y_hat_batch = X_test_batch.to(self.device), y_hat_batch.to(self.device)
            if self.config.model_architecture == "LSTM":
