...
class Detector:
    ...
    def __init__(
        self, labels_path=str | None, result_path="results/", config_path="config.yaml"
    ):
        """Top-level class for running anomaly detection over a group of channels with
        values stored in .npy files. Also evaluates performance against a set of labels
        if provided.
    ...
    def log_final_stats(self):
        ...
        if self.labels_path:
            ...
            logger.info("-----------------")
            logger.info(
                "True Positives: {}".format(self.result_tracker["true_positives"])
            )
            logger.info(
                "False Positives: {}".format(self.result_tracker["false_positives"])
            )
            logger.info(
                "False Negatives: {}".format(self.result_tracker["false_negatives"])
            )
            try:
            ...
            logger.info(
                "Total anomalies found: {}".format(
                    self.result_df["n_predicted_anoms"].sum()
                )
            )
            logger.info(
                "Avg normalized prediction error: {}".format(
    ...
    def run_stage2_retrain(self):
        ...
        result_stage1_csv_files = glob.glob(os.path.join(self.result_path, "*.csv"))
            ...
            with open(csv_files, "r", encoding="utf-8") as csvfile:
                ...
                with open(
                    ...
                    for index, row in self.chan_df.iterrows():
                        ...
                        tot_test_mes += mse
                        print(
                            f"Mean Squared Error for channel {row['chan_id']} : {mse} {tot_test_mes}"
                        )
                        ...
                        if self.labels_path:
                            ...
                            logger.info(
                                "Total true positives: {}".format(
                                    self.result_tracker["true_positives"]
                                )
                            )
                            logger.info(
                                "Total false positives: {}".format(
                                    self.result_tracker["false_positives"]
                                )
                            )
                            logger.info(
                            ...
                            logger.info(
                                "{} anomalies found".format(
                                    result_row["n_predicted_anoms"]
                                )
                            )
                            logger.info(
                                "anomaly sequences start/end indices: {}".format(
                                    result_row["anomaly_sequences"]
                                )
                            )
                            logger.info(
                                "number of test values: {}".format(
